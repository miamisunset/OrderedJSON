...
@Test func flattenEmptyObject() {
    ...
    guard case let .object(dict) = result.storage else {
...
@Test func flattenKeyWithSlash() {
    ...
    guard case let .object(dict) = flat.storage else {
        Issue.record("Expected object")
        return
    }
...
@Test func unflattenEscapedKeys() throws {
    ...
    guard case let .object(dict) = unflattened.storage else {
        Issue.record("Expected object")
        return
    }
...
@Test func flattenUnflattenRoundTripWithEmptyContainers() throws {
    ...
    guard case let .object(dict) = reconstructed.storage else {
        Issue.record("Expected object")
        return
    }
...
@Test func unflattenNonPrimitiveValueThrows() throws {
    ...
    #expect {
        ...
        guard let flattenErr = error as? FlattenError else { return false }
        if case let .notPrimitive(key, type) = flattenErr {
