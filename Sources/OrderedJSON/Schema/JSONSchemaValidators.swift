...
extension JSONSchema {
    ...
    func validatePropertyNames(
        ...
        guard case let .object(dict) = value.storage else { return }
        for (key, _) in dict {
    ...
    func validatePatternProperties(
        ...
        guard case let .object(patternDict) = pp.storage, case let .object(dict) = value.storage else {
            return
        }
    ...
    func evaluatedPropertyKeys(
        ...
        if let properties = subschema["properties"], properties.isObject {
            guard case let .object(props) = properties.storage else {
        ...
        if let pp = subschema["patternProperties"], pp.isObject {
            guard case let .object(patternDict) = pp.storage else {
    ...
    func evaluatedPropertyKeysRecursive(
        ...
        if let depSchemas = subschema["dependentSchemas"], depSchemas.isObject {
            guard case let .object(depDict) = depSchemas.storage else { return keys }
    ...
    func validateDependencies(
        ...
        guard case let .object(depDict) = deps.storage else { return }
    ...
    func validateDependentRequired(
        ...
        guard case let .object(depDict) = depRequired.storage else { return }
