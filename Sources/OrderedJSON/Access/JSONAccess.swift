...
public extension JSON {
    ...
    var count: Int {
        ...
        case let .array(arr): return arr.count
    ...
    var isEmpty: Bool {
        switch storage {
        case let .object(dict): return dict.isEmpty
        case let .array(arr): return arr.isEmpty
    ...
    var first: JSON? {
        switch storage {
        case let .array(arr): return arr.first
    ...
    var last: JSON? {
        switch storage {
        case let .array(arr): return arr.last
        case let .object(dict):
    ...
    /// For arrays and primitives, returns `nil`.
    var objectValue: OrderedDictionary<String, JSON>? {
        guard case let .object(dict) = storage else { return nil }
        return dict
    }
}
