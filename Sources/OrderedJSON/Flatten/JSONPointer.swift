...
public enum JSONPointerError: Error, Hashable, Sendable, CustomStringConvertible {
    ...
    public var description: String {
        switch self {
        case let .invalidSyntax(msg): return "Invalid JSON Pointer syntax: \(msg)"
        case let .missingValue(ptr): return "JSON Pointer '\(ptr)' references a nonexistent value"
        case let .leadingZero(seg): return "Array index with leading zero: '\(seg)'"
        }
    }
}
...
public struct JSONPointer: Hashable, Sendable {
    ...
    public func resolve(_ json: JSON) -> JSON? {
        ...
        for segment in segments {
            ...
            if let index = Int(segment) {
                guard case let .array(arr) = current.storage else { return nil }
                guard index >= 0, index < arr.count else { return nil }
                current = arr[index]
            } else {
                guard case let .object(dict) = current.storage else { return nil }
    ...
    public func resolveOrThrow(_ json: JSON) throws -> JSON {
        ...
        for segment in segments {
            ...
            if let index = Int(segment) {
                guard case let .array(arr) = current.storage else {
                    throw JSONPointerError.missingValue(description)
                }
                ...
                guard case let .object(dict) = current.storage else {
                    throw JSONPointerError.missingValue(description)
                }
...
extension JSON {
    ...
    static func setJSONPointerPath(into json: inout JSON, parts: [String], value: JSON) {
        ...
        if first == "-" {
            ...
            if case var .array(arr) = json.storage {
                if rest.isEmpty {
                    arr.append(value)
                } else {
            ...
            // Object path segment
            if case var .object(dict) = json.storage {
                if rest.isEmpty {
                    dict[first] = value
                } else {
                    if dict[first] == nil {
