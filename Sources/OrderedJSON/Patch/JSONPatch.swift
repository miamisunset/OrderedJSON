...
extension JSON {
    ...
    public mutating func patchInPlace(_ patchValue: JSON) throws {
        guard case let .array(operations) = patchValue.storage else {
    ...
    private mutating func applyPatchOperation(_ op: JSON) throws {
        guard case let .object(dict) = op.storage else {
    ...
    private func resolvePointer(segments: [String], in json: JSON) -> JSON? {
        ...
        for segment in segments {
            ...
            if let index = Int(segment) {
                guard case let .array(arr) = current.storage,
                      index >= 0, index < arr.count
                else { return nil }
                current = arr[index]
            } else {
                guard case let .object(dict) = current.storage,
    ...
    private func traverseAndSet(
        ...
        if segment == "-" {
            guard case let .array(arr) = json.storage else {
        ...
        if let idx = Int(segment) {
            ...
            guard case var .object(dict) = json.storage else {
